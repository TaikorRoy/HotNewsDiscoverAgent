...
class HotNewsDiscoverAgent(object):
    def __init__(self, _news_pool_file_path, _obsolete_pool_file_path, _interval):
    ...
    def run_one_job(self):
        pool_lib.update_news_pool(self.news_pool_file_path)
        pool_lib.update_obsolete_pool(self.obsolete_pool_file_path)

        hot_news_and_kws = hot_news_picker_lib.select_hot_news(self.news_pool_file_path, self.obsolete_pool_file_path)
...
if __name__ == "__main__":
    ...
    interval = 1*3600
    agent = HotNewsDiscoverAgent(news_pool_file_path, obsolete_pool_file_path, interval)
    agent.run_batch()
